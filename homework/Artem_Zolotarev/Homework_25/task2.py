...
def test_registration_form(driver):
    ...
    phone.send_keys('9' + str(random.randint(10 ** 9, 10 ** 10 - 2)))
    ...
    state.click()
    state_element = wait.until(EC.element_to_be_clickable((By.XPATH, '//*[@id="react-select-3-option-1"]')))
    state_element.click()
    city = wait.until(EC.visibility_of_element_located((By.XPATH, '//*[@class=" css-1wa3eu0-placeholder"]')))
    city.click()
    city_element = wait.until(EC.element_to_be_clickable((By.XPATH, '//*[@id="react-select-4-option-1"]')))
    city_element.click()
    submit_btn = driver.find_element(By.XPATH, '//button[@id="submit"]')
    submit_btn.click()
    modal_body = driver.find_element(By.CLASS_NAME, 'modal-body')
    modal_body.send_keys(Keys.ENTER)
